    ...
    def __init__(self, sqlite_cursor=None, sqlite_row=None, line=None, srcdict=None):
        if srcdict is not None:
            ...
            if line[0] != '[' and line[11] != ']':
                ...
                attempt, state = [0] * 2
                command_name, comment, contact_name, host_name, \
                    message, plugin_output, service_description, state_type = [''] * 8
                time = line[1:11]
                first_type_pos = line.find(' ') + 1
                last_type_pos = line.find(':')
                first_detail_pos = last_type_pos + 2

                line_type = line[first_type_pos:last_type_pos]
                options = line[first_detail_pos:]
                message = line

                if line_type == 'CURRENT SERVICE STATE':
                ...
                elif line_type == 'INITIAL SERVICE STATE':
                    logobject = LOGOBJECT_SERVICE
                    logclass = LOGCLASS_STATE
                    host_name, service_description, state, state_type, attempt, plugin_output = options.split(';', 5)

                elif line_type == 'SERVICE ALERT':
                ...
                elif line_type == 'SERVICE DOWNTIME ALERT':
                    logobject = LOGOBJECT_SERVICE
                    logclass = LOGCLASS_ALERT
                    host_name, service_description, state_type, comment = options.split(';', 3)

                elif line_type == 'SERVICE FLAPPING ALERT':
                    logobject = LOGOBJECT_SERVICE
                    logclass = LOGCLASS_ALERT
                    host_name, service_description, state_type, comment = options.split(';', 3)

                elif line_type == 'CURRENT HOST STATE':
                    logobject = LOGOBJECT_HOST
                    logclass = LOGCLASS_STATE
                    host_name, state, state_type, attempt, plugin_output = options.split(';', 4)

                elif line_type == 'INITIAL HOST STATE':
                    logobject = LOGOBJECT_HOST
                    logclass = LOGCLASS_STATE
                    host_name, state, state_type, attempt, plugin_output = options.split(';', 4)

                elif line_type == 'HOST ALERT':
                    logobject = LOGOBJECT_HOST
                    logclass = LOGCLASS_ALERT
                    host_name, state, state_type, attempt, plugin_output = options.split(';', 4)
                    state = host_states[state]

                elif line_type == 'HOST DOWNTIME ALERT':
                    logobject = LOGOBJECT_HOST
                    logclass = LOGCLASS_ALERT
                    host_name, state_type, comment = options.split(';', 2)

                elif line_type == 'HOST FLAPPING ALERT':
                    logobject = LOGOBJECT_HOST
                    logclass = LOGCLASS_ALERT
                    host_name, state_type, comment = options.split(';', 2)

                elif line_type == 'SERVICE NOTIFICATION':
                    ...
                    contact_name, host_name, service_description, state_type, command_name, plugin_output = \
                        options.split(';', 5)
                    if '(' in state_type:  # downtime/flapping/etc-notifications take the type UNKNOWN
                        state_type = 'UNKNOWN'
                    state = service_states[state_type]

                elif line_type == 'HOST NOTIFICATION':
                    # tust_cuntuct;test_host_0;DOWN;notify-host;i am DOWN
                    logobject = LOGOBJECT_HOST
                    logclass = LOGCLASS_NOTIFICATION
                    contact_name, host_name, state_type, command_name, plugin_output = options.split(';', 4)
                    if '(' in state_type:
                        state_type = 'UNKNOWN'
                    state = host_states[state_type]

                elif line_type == 'PASSIVE SERVICE CHECK':
                    logobject = LOGOBJECT_SERVICE
                    logclass = LOGCLASS_PASSIVECHECK
                    host_name, service_description, state, plugin_output = options.split(';', 3)

                elif line_type == 'PASSIVE HOST CHECK':
                    logobject = LOGOBJECT_HOST
                    logclass = LOGCLASS_PASSIVECHECK
                    host_name, state, plugin_output = options.split(';', 2)

                elif line_type == 'SERVICE EVENT HANDLER':
                ...
                elif line_type == 'HOST EVENT HANDLER':
                    logobject = LOGOBJECT_HOST
                    host_name, state, state_type, attempt, command_name = options.split(';', 4)
                    state = host_states[state]

                elif line_type == 'EXTERNAL COMMAND':
                    logobject = LOGOBJECT_INFO
                    logclass = LOGCLASS_COMMAND

                elif (line_type.startswith('starting...') or line_type.startswith('shutting down...')
                      or line_type.startswith('Bailing out') or line_type.startswith('active mode...')
                      or line_type.startswith('standby mode...') or line_type.startswith('Warning')):
                    logobject = LOGOBJECT_INFO
    ...
    def fill(self, datamgr):
        ...
        if hasattr(self, 'logobject') and self.logobject == LOGOBJECT_HOST:
            try:
                ...
                logger.error("[Livestatus Log Lines] Error on host: %s", str(exp))
            ...
            try:
                ...
                logger.error("[Livestatus Log Lines] Error on service: %s", str(exp))
