...
from livestatus_query_metainfo import HINT_HOST, HINT_HOSTS, HINT_SERVICES_BY_HOST, \
    HINT_SERVICE, HINT_SERVICES_BY_HOSTS, HINT_SERVICES, \
...
def itersorted(self, hints=None):
    ...
    if hints is None:
        ...
        try:
            ...
            logger.error("[Livestatus Regenerator] Hint_services exception: %s", str(exp))

    elif hints['target'] == HINT_SERVICES_BY_HOSTS:
        try:
            preselected_ids = [
                _id for h in hints['host_name'] if h in self._id_by_host_name_heap
                for _id in self._id_by_host_name_heap[h]]
            preselection = True
        except Exception:
            pass
    ...
    if 'authuser' in hints:
        if preselection:
            try:
                for _id in [pid for pid in preselected_ids if pid in self._id_contact_heap[hints['authuser']]]:
            ...
            try:
                for _id in self._id_contact_heap[hints['authuser']]:
                    yield self.items[_id]
            except Exception:
                pass
    else:
...
class LiveStatusRegenerator(Regenerator):
    ...
    def all_done_linking(self, inst_id):
        ...
        [
            ...
            for (k, v) in self.hosts.items.iteritems() for c in v.contacts
        ]
        ...
        if self.service_authorization_strict:
            [
                ...
                for (k, v) in self.services.items.iteritems() for c in v.contacts
            ]
        else:
            # 1. every host contact automatically becomes a service contact
            [
                self.services._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k)
                for (k, v) in self.services.items.iteritems() for c in v.host.contacts
            ]
        ...
        [
            self.services._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k)
            for (k, v) in self.services.items.iteritems() if not v.contacts for c in v.host.contacts
        ]
        ...
        if self.group_authorization_strict:
            ...
            [
                ...
                for (k, v) in self.hostgroups.items.iteritems() for h in v.members for c in h.contacts
            ]
            ...
            [
                ...
                for (k, v) in self.servicegroups.items.iteritems() for s in v.members for c in s.contacts
            ]
        ...
        setattr(self.hosts, '_id_by_host_name_heap',
                dict([(get_obj_full_name(v), k) for (k, v) in self.hosts.items.iteritems()]))
        setattr(self.services, '_id_by_service_name_heap',
                dict([(get_obj_full_name(v), k) for (k, v) in self.services.items.iteritems()]))
        ...
        setattr(self.hostgroups, '_id_by_hostgroup_name_heap',
                dict([(get_obj_full_name(v), k) for (k, v) in self.hostgroups.items.iteritems()]))
        setattr(self.servicegroups, '_id_by_servicegroup_name_heap',
                dict([(get_obj_full_name(v), k) for (k, v) in self.servicegroups.items.iteritems()]))
        ...
        [
            ...
            for (k, v) in self.hosts.items.iteritems() for hg in v.hostgroups
        ]
        ...
        [
            ...
            for (k, v) in self.services.items.iteritems() for sg in v.servicegroups
        ]
        ...
        [
            ...
            for (k, v) in self.services.items.iteritems() for hg in v.host.hostgroups
        ]
